...
fn try_ssl_write(ctx: ProbeContext) -> Result<u32, i64> {
    ...
    // Phân loại method
    let method: u8 = if read_len >= 4 && &buf[..4] == b"GET " {
        HttpMethod::GET as u8
    } else if read_len >= 5 && &buf[..5] == b"POST " {
        HttpMethod::POST as u8
    } else if read_len >= 4 && &buf[..4] == b"PUT " {
        HttpMethod::PUT as u8
    } else if read_len >= 7 && &buf[..7] == b"DELETE " {
        HttpMethod::DELETE as u8
    } else if read_len >= 16 && &buf[..16] == b"PRI * HTTP/2.0\r\n" {
        0xF0 // HTTP/2 preface — đánh dấu để userspace biết
    } else if read_len >= 9 {
        // Kiểm tra có phải HTTP/2 binary frame không
        // frame type ở byte [3]: 0x00=DATA, 0x01=HEADERS, 0x04=SETTINGS...
        let ftype = buf[3];
        if ftype <= 0x09 {
            0xE0 | ftype // 0xE0=DATA, 0xE1=HEADERS, 0xE4=SETTINGS...
        } else {
            return Ok(0); // unknown, bỏ qua
        }
    } else {
        return Ok(0);
    };
    ...
    if let Some(mut entry) = RING_BUF_REQ.reserve::<HttpEvent>(0) {
        ...
        unsafe {
            core::ptr::copy_nonoverlapping(buf.as_ptr(), event.data.as_mut_ptr(), read_len);
            core::ptr::write_bytes(event.data.as_mut_ptr().add(read_len), 0, MAX_DATA - read_len);
        }
        entry.submit(0);
    }
    Ok(0)
...
fn try_ssl_read_ret(ctx: RetProbeContext) -> Result<u32, i64> {
    ...
    if buf_ptr.is_null() { return Ok(0); }
...
fn try_skb_copy_datagram_iter(ctx: ProbeContext) -> Result<u32, i64> {
    ...
    if sk_common.skc_family != AF_INET { return Ok(0); }

    let data_ptr: *const u8 = unsafe {
        bpf_probe_read_kernel(&(*skb).data as *const *mut u8)? as *const u8
    };
    if data_ptr.is_null() { return Ok(0); }

    const MAX_DATA: usize = 496;
    let read_len = (len as usize).min(MAX_DATA);
    if read_len < 5 { return Ok(0); }

    let buf = unsafe {
        let ptr = BUF.get_ptr_mut(0).ok_or(1i64)?;
        &mut *ptr
    };
    unsafe { bpf_probe_read_kernel_buf(data_ptr, &mut buf[..read_len])? };

    if &buf[..5] != b"HTTP/" { return Ok(0); }
...
fn try_tcp_sendmsg(ctx: ProbeContext) -> Result<u32, i64> {
    ...
    let dest_addr = u32::from_be(unsafe { sk_common.__bindgen_anon_1.__bindgen_anon_1.skc_daddr });
    let src_port  = u16::from_be(unsafe { sk_common.__bindgen_anon_3.__bindgen_anon_1.skc_num });
    let dest_port = u16::from_be(unsafe { sk_common.__bindgen_anon_3.__bindgen_anon_1.skc_dport });
    ...
    if read_len < 4 { return Ok(0); }

    let buf = unsafe {
        let ptr = BUF.get_ptr_mut(0).ok_or(1i64)?;
        &mut *ptr
    };
    unsafe { bpf_probe_read_user_buf(iovec.iov_base as *const u8, &mut buf[..read_len])? };

    let method = match buf[0] {
        b'G' => HttpMethod::GET as u8,
        b'P' => {
            if read_len >= 4 && buf[1] == b'O' { HttpMethod::POST as u8 }
            else { HttpMethod::PUT as u8 }
        }
